class StageupOverlay extends EgretGameObject {

  private newStage: eui.Label;
  ...
  constructor() {
    ...
    this.newStage = createLabel("99", { type: "stage", valign: "center", halign: "center" }, 240, 0x333322, true);
    this.addEgretDisplayObject(this.newStage);

    const stageUp = createLabel("次のステージに\n進みました", { type: "next-above", align: "center", anchor: this.newStage, offsetY: -30 }, 60, 0x333322);
    this.addEgretDisplayObject(stageUp);

    this.tapToStart = createLabel("タップする", { type: "next-below", anchor: this.newStage, align: "center", offsetY: 60 }, 60, 0x333322, true);
  ...
  show(result: Result, callback: { fn: Function, thisObject: any }): void {
    setLabelText(this.newStage, `${result.stage}`);
